...
class UVdbSequenceComponent : public UVdbComponentBase, public IInterface_StreamableVolumetricAssetOwner
{
	GENERATED_UCLASS_BODY()

	virtual ~UVdbSequenceComponent();

	// If FogVolume, represents Density values. If LevelSet, represents a narrow-band level values
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Playback", meta = (EditCondition = "Autoplay"))
	bool Looping = true;

	// Speed at which the animation is playing
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Playback", meta = (UIMin = 0.0001, ClampMin = 0.0001, EditCondition = "Autoplay"))
	float PlaybackSpeed = 1.0f;

	// Duration of the sequence
	UPROPERTY(VisibleAnywhere, transient, Category = "Playback")
	float Duration = 0.f;

	// Sequence start offset, in relative range [0, 1] where 0 represents the start and 1 the end of the sequence.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Playback", meta = (DisplayName="Offset", UIMin = 0.0, UIMax = 1.0, ClampMin = 0.0, ClampMax = 1.0))
...
private:
	bool UpdateSceneProxy(uint32 FrameIndex);
